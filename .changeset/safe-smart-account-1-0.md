...
- `sign({ hash })` now produces a verifier-compatible ERC-1271 signature: the raw 32-byte hash goes through the `SafeMessage` wrapper (`eth_sign` flavour, like `signMessage`), so `client.verifyHash` accepts it (0.x signed the hash's hex string, which no verifier accepted).
